// Detekcia buniek v Luffyho JUS sheete (kolaz na jednofarebnom pozadi): node detect.cjs
// Connected components -> merge blizkych boxov -> riadky -> ocislovany overview + cells json
// (prevzate z tools/sprites-jotaro/detect_jus.cjs; navyse minH filter na textove popisky
// a exclude() na artwork v rohu namiesto globalneho maxX)
const fs = require("fs");
const path = require("path");
const { PNG } = require("pngjs");
const SHEETS = require("./sheet.cjs");

const cfg = SHEETS[process.argv[2] || "luffy"];
...
// vyhod artwork v rohu, textove popisky (nizke boxy) a obrie zliatky
items = items.filter(b =>
  !(cfg.exclude && cfg.exclude(b)) &&
  (b.y1 - b.y0 + 1) >= (cfg.minH || 0) &&
  (b.x1 - b.x0) < (cfg.maxW || 400) &&
  (b.y1 - b.y0) < (cfg.maxH || 400));
