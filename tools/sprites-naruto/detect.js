// Detekcia frame-ov v NARUTO.png: sprite = pixel, ktory nie je magenta cell-bg ani sedy outer-bg.
// Connected components -> merge blizkych boxov -> zoradenie do riadkov -> ocislovany overview + cells.json
const fs = require("fs");
const path = require("path");
const { PNG } = require("pngjs");

const SRC = path.join(__dirname, "NARUTO.png");
const OUT = __dirname;
...
function isBg(r, g, b) { return isMagenta(r, g, b) || isGray(r, g, b); }

// --- connected components (8-conn) nad sprite pixelmi
const label = new Int32Array(W * H).fill(-1);
const boxes = []; // {x0,y0,x1,y1,count}
const stack = [];
for (let y = 0; y < H; y++) {
  for (let x = 0; x < W; x++) {
    const idx = y * W + x;
    ...
    const [r, g, b] = at(x, y);
    if (isBg(r, g, b)) { label[idx] = -2; continue; }
    // novy komponent
    ...
    while (stack.length) {
      ...
      for (let dy = -1; dy <= 1; dy++) for (let dx = -1; dx <= 1; dx++) {
        ...
        if (isBg(nr, ng, nb)) { label[ni] = -2; continue; }
...
// --- merge boxov s medzerou <= GAP (iterativne)
const GAP = 6;
let items = boxes.filter(b => b.count >= 12); // sum drobnych pixelov prec
let merged = true;
while (merged) {
  merged = false;
  outer:
...
// --- zoradenie do riadkov podla prekryvu y
items.sort((a, b) => a.y0 - b.y0 || a.x0 - b.x0);
const rows = [];
for (const it of items) {
  const cy = (it.y0 + it.y1) / 2;
  let row = rows.find(r => cy >= r.y0 && cy <= r.y1 + 8);
...
// --- overview s cislami (mini 3x5 font pre cislice a bodku)
const FONT = {
...
fs.writeFileSync(path.join(OUT, "overview.png"), PNG.sync.write(ov));
fs.writeFileSync(path.join(OUT, "cells.json"), JSON.stringify(cellsOut, null, 1));
