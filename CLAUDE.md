...
Client → server: `configure_match` (host only, in `lobby` phase), `choose_character` (fire | lightning | wanderer — only in `playing` phase, once per game), `lock_in` (array of 3 actions, or 4 with `golden_shield`/`golden_mirror` first — only valid from the round's non-starter; only in `playing` phase), `retry`, `admin_reset_all`.
Server → client: `you_are` (`{ slot, isHost }`; slot is fixed for the series), `state` (snapshot incl. `phase`/`config`/`series`, optionally with `timeline`), `game_result` (per-game outcome), `new_game` (next game in series — reset round UI, keep score), `game_over` (series decided), `reset`.
